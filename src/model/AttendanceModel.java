...
public class AttendanceModel {
    ...
    public static ArrayList<AttendenceTM> getAllAttendance(String dateId) throws SQLException, ClassNotFoundException {
        ResultSet rs = CrudUtil.execute("SELECT e.name , a.status from attendence a left join employee e on a.employee_id = e.id where a.day_id = ?\n" +
                "\n", dateId);
        ArrayList<AttendenceTM> list = new ArrayList<>();
        while (rs.next()){
            list.add(new AttendenceTM(rs.getString(1),rs.getString(2)));
        }
        return list;
    }
